...
namespace DiamondLuxurySolution.AdminCrewApp.Service.Platform
{
    public class PlatformApiService : BaseApiService, IPlatformApiService
    {
        ...
        {
            var data = await PostAsyncHasImage<bool>("api/Platforms/Create",request);
            return data;
        }

        public async Task<ApiResult<bool>> DeletePlatform(DeletePlatformRequest request)
        {
            var data = await DeleteAsync<bool>($"api/Platforms/Delete?platformId={request.PlatformId}");
            return data;
        }
        ...
        {
            var data = await GetAsync<PlatfromVm>($"api/Platforms/GetById?PlatformId={PlatformId}");
            return data;
        }

        public async Task<ApiResult<bool>> UpdatePlatform(UpdatePlatformRequest request)
        {
            var data = await PutAsyncHasImage<bool>("api/Platforms/Update",request);
            return data;
        }
        ...
        {
            var data = await GetAsync<PageResult<PlatfromVm>>($"api/Platforms/ViewInManager?Keyword={request.Keyword}&pageIndex={request.pageIndex}");
            return data;
        }
    }
}
