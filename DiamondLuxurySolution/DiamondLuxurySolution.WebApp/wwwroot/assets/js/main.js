(function ($) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const decreaseButton = document.querySelector(".quantity-decrease1");
    const increaseButton = document.querySelector(".quantity-increase1");
  ...
  document.addEventListener("DOMContentLoaded", (event) => {
    ...
    decreaseButton.addEventListener("click", () => {
      let currentValue = parseInt(quantityInput.value);
      if (currentValue > 1) {
        // Change this value if you want to allow 0 or negative values
        quantityInput.value = currentValue - 1;
      }
    });
  ...
  function cartDrpdownExpand() {
    $("#cartDropdown").on("click", function (e) {
      ...
      e.stopPropagation();
      $(this)
        .siblings(".mini-cart__dropdown-menu")
        .addClass("open")
        .slideToggle(500);
  ...
  homeSlider.on("init", function (e, slick) {
    var $firstAnimatingElements =
      $("div.single-slider").find("[data-animation]");
  ...
  $(".js-product-carousel-2").slick({
    ...
    arrows: true,
    prevArrow:
      '<span class="slick-btn slick-prev"><i class="fa fa-angle-left"></i></span>',
    nextArrow:
      '<span class="slick-btn slick-next"><i class="fa fa-angle-right"></i></span>',
    responsive: [
      {
        breakpoint: 1200,
        settings: {
